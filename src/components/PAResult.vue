<template>
  <div class="wraper">
    <el-select v-model="dialog.season" placeholder="请选择被评价人" @change="seasonChangeHandler(dialog)">
      <el-option
        v-for="item in seasonOptions" :key="item.value"
        :label="item.label"
        :value="item.value">
      </el-option>
    </el-select>
...
<script>
  export default {
    ...
    methods:{
      ...
      seasonChangeHandler(dialog){
        let self = this;
        self.showUserData = [];
        let season = dialog.season;
